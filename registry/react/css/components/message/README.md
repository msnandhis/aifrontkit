...
<Message message={message} variant="conversation" />
```
...
For complete replacement, pass children. To replace one content kind, use
`partComponents`; the default file renderer uses the compound `File` component.
The primitive keeps the message article and a single polite status announcement;
streamed text itself is not a live region, avoiding token-by-token screen-reader noise.
...
```css
:root {
  --aifk-user-surface: var(--app-accent-subtle);
  --aifk-surface: var(--app-panel);
  --aifk-border: var(--app-border);
