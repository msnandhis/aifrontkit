...
function Root({ toolCallId, children, ...props }: PropsWithChildren<{ toolCallId: string } & ComponentPropsWithoutRef<"section">>) {
  ...
  if (!tool) return null;
  return <ToolContext.Provider value={tool}><section data-aifk-tool="" data-status={tool.status} aria-busy={tool.status === "running"} {...props}>{children}</section></ToolContext.Provider>;
}
...
export const ToolPrimitive = { Root, Name, Status, Output };
