...
import { createTheme, toCssVariables } from "../dist/theme/tokens.js";

const packageRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const sourceFile = resolve(packageRoot, "src/theme.css");
const distributionFile = resolve(packageRoot, "dist/theme.css");
...
const sections = [
  "/* This file is generated from src/theme/tokens.ts. Run `pnpm --filter @aifrontkit/react build`; do not hand-edit. */",
...
if (checkOnly) {
  ...
  if (current !== output) {
    console.error("Generated theme CSS is stale. Run `pnpm --filter @aifrontkit/react build`.");
    process.exitCode = 1;
  }
} else {
