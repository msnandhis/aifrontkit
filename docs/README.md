# AIFrontKit engineering documentation

This directory is the repository's internal source of truth for architecture,
engineering policy, decisions, and product strategy. It is written for
maintainers and contributors rather than used as the public documentation site.

Public, publishable product documentation lives in [`../content/docs`](../content/docs/README.md).
That content has its own navigation and version manifests and may be imported by
the platform without exposing private operational documentation.

Each engineering capability has one primary document. Cross-cutting documents
link to capabilities rather than redefining them.

## Foundation

- [End-to-end architecture](./architecture.md)
- [Schemas and event model](./schemas-and-event-model.md)
- [Client runtime](./client-runtime.md)
- [Transports and adapters](./transports-and-adapters.md)
- [Planned codebase structure](./planned-codebase-structure.md)

Cross-repository TypeScript, JavaScript output, loading, and performance-budget decisions are defined in [Technology and performance strategy](./strategy/technology-and-performance.md).

## Interface system

- [Primitives and components](./primitives-and-components.md)
- [Multimodal content](./multimodal-content.md)
- [Artifacts](./artifacts.md)
- [Interaction patterns](./interaction-patterns.md)
- [Workspaces and experiences](./workspaces-and-experiences.md)
- [Theming](./theming.md)
- [Variants and customization](./variants-and-customization.md)
- [Visual system and product-quality defaults](./visual-system.md)
- [Framework adapters](./framework-adapters.md)

## Distribution and authoring

- [Registry](./registry.md)
- [CLI](./cli.md)
- [Playgrounds and examples](./playgrounds-and-examples.md)
- [Documentation system](./documentation-system.md)
- [Studio schema](./studio-schema.md)

## Quality and stewardship

- [Accessibility](./accessibility.md)
- [Performance](./performance.md)
- [Security](./security.md)
- [Testing and quality](./testing-and-quality.md)
- [Releases and compatibility](./releases-and-compatibility.md)
- [Contribution and governance](./contribution-and-governance.md)

Shared scope, licensing, repository ownership, and roadmap live in the [product strategy documentation](./strategy/README.md).
